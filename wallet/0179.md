...
1. Sort the Capabilities by `id` using byte/codepoint ascending order.
...
4. **Canonicalize** the preimage to a string by serializing as JSON with: keys sorted ascending at every object level, no insignificant whitespace (item separator `","`, key/value separator `":"`), and all non-ASCII characters `\uXXXX`-escaped (ASCII-only output). Under this canonicalization the outer object serializes with keys in the order `capabilities`, `domain`, and each projection object serializes with keys in the order `enabled`, `id`, `tier`.
5. Encode the canonical string as ASCII bytes and compute `SHA-256`. The fingerprint is the lowercase hex digest.

Only JSON strings and booleans appear in the preimage, so the canonicalization is unambiguous without a number-formatting rule; implementations that use a different canonical-JSON library MUST reproduce the exact byte sequence described here (this is the specific form emitted by a sorted-keys, tight-separator, ASCII-escaped JSON serializer, it is *not* required to be RFC 8785 JCS, but the two agree on this preimage).
...
The runtime enforcer is constructed with a `pinned_fingerprint` value that is **baked into the build artifact** (compiled in / embedded at build time), not read from config, environment, or the manifest file itself.

- At construction the enforcer MUST recompute the fingerprint of the manifest handed to it and, if `pinned_fingerprint` is non-null and does not byte-equal the recomputed fingerprint, MUST refuse to construct (raise a pin error). This guarantees that a swapped or edited manifest file, a config value, or an environment variable **cannot widen the exposed surface at runtime**: the only manifest an enforcer will run is one that fingerprints to the value fixed at build time.
