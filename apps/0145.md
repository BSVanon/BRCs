...
**Adopter-name profile.** An adopter name MUST be printable UTF-8, MUST NOT contain a NUL byte, and MUST NOT begin with `nkan/`. The last constraint closes a cross-context parse. `u8(len(adopter_bytes))` is a single length byte, and for an adopter name of the right length beginning `nkan/` that byte can equal `0x69`, ASCII `i`, so the fingerprint preimage would open with the exact nine bytes of `INKAN_DOMAIN` and would also parse as a glyph seed under section 2. One digest would then be simultaneously a valid fingerprint and a valid anchor.
...
- `MALFORMED`, the record, the public key, or the signature did not parse, so no signature check was performed. A verifier MUST report `MALFORMED` whenever its inputs did not parse, and MUST NOT report `INVALID` in that case. `INVALID` asserts that a check ran and the signature was wrong, which is an accusation of forgery; a truncated key or a DER blob that will not decode is a handling error, and reporting it at maximum severity trains an operator to discount the verdict that matters.
- `UNSUPPORTED`, the record parsed and carries a record version `v` this verifier does not implement. A verifier MUST check `v`. It MUST NOT report `OK` for an unknown version, which would assert a validity it is not able to judge, and MUST NOT report `INVALID`, which would assert a failure it did not observe. A well-formed, correctly signed, fresh `v=2` record read by a `v=1` verifier is the case this rule exists for.
...
A record whose shape does not match the 7.2 table, exactly those fields with those types, is `MALFORMED`, and a verifier MUST validate the shape before it reports `OK`. The signature covers whatever fields are present, so a record of the wrong shape is not a forgery. It can still be published by mistake, and without this rule every verifier reports `OK` over it while downstream readers disagree about what they have read.

For a canary, absence, `STALE`, `INVALID`, `MALFORMED`, or `UNSUPPORTED` is itself the warning.
...
- **Erosion of the forgery figures.** Every visual-forgery cost above is stated in 2026 classical compute and decays. Ordinary hardware growth lowers the classical cost over a decade, and Grover search takes a birthday bound of `2^n` renders toward `2^(n/2)` for an adversary holding a cryptographically relevant quantum computer, which would reduce the `t1` figure of `2^40` toward the order of `2^20`. These figures bound how hard a picture is to counterfeit; they are not the security of the standard. The 32-byte anchor is the trust root, a verifier re-derives it, and none of the guarantees in this section rest on the seal being hard to draw.
