...
registry at `Tier=0xFF`, with `Preference` assigned by position in the
configured list so that the operator's ordering is preserved among seeds: the
first seed takes the highest preference and each subsequent seed one lower.
A single shared preference would leave seeds tied under the sort below, making
the fallback order arbitrary.
...
| ----------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Cache TTL (per class)   | Retry endpoint | Frames expire naturally; bounds retransmit window. The TTL is set per frame class, since a block or subtree stays useful far longer than a transaction |
...
| `MsgTypeTHROTTLED`      | 19         | `0x13`       | THROTTLED congestion response          |
...
| `DefaultCacheTTL`       | 60         | —            | Default cache TTL for transaction frames (s); other frame classes carry their own longer defaults |
