...
services that are not general-purpose transaction sharding. Implementations
MUST NOT join or transmit to addresses in this range unless a BRC explicitly
assigns them.

[BRC-148](./0148.md) is such a BRC: it partitions this range into object
planes by the high nibble of the index and allocates the first of them. This
section is superseded to that extent. Every other provision of this document,
including the IANA allocation, the group-id field and the control-plane
indices, is unchanged by it.
