...
**Deprecated, and retained deliberately.** Standalone coinbase carriage, the
mechanism this BRC specifies (BRC-131 `MsgType 0x02`, `BlockMsgCoinbase`), is
deprecated. It is kept on purpose and is not withdrawn.

- **Not produced.** A coinbase transaction is invalid to a node unless it is
  connected to its block, so current implementations never emit standalone
  coinbase frames. The coinbase travels inline in the block body defined by
  [BRC-144](./0144.md), where it inherits the proof of work of the block it
  belongs to.
- **Dropped by default.** A standalone coinbase frame carries no proof of work
  of its own, so nothing about it can be validated in isolation. A listener
  that gates block-control frames before fan-out (the reference listener's
  block-control gate, which is on by default) drops such a frame; the
  reference implementation counts it under the drop reason `coinbase_legacy`.
- **Retained deliberately.** A future design may need to carry blocks and
  their coinbase separately across the multicast fabric and recombine them at
  the edges. This specification keeps that option open, so it remains the
  reference for such a design, for deployments that disable the gate, and for
  decoding archived traffic.
- **Reserved.** The BRC-133 number, BRC-131 `MsgType 0x02`, and the virtual
  flow index `0xFFF8` (`GroupCoinbaseFlow`) remain reserved for standalone
  coinbase carriage. They must not be reused for any other purpose.
...
| `BlockMsgCoinbase`    | `0x02`   | MsgType: raw coinbase transaction (deprecated; reserved, see Status Note)      |
| `GroupBlockBroadcast` | `0xFFFE` | Block control multicast group index (egress)                                   |
| `GroupCoinbaseFlow`   | `0xFFF8` | Virtual HashKey ingredient; coinbase flow identity (never a multicast address); reserved |
...
- [BRC-144: Block Frame Format](./0144.md): the block body that carries the
  coinbase inline, superseding standalone coinbase carriage
