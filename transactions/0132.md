...
- Retry endpoints join `FF0X::B:FFFB` and cache BRC-132 frames and BRC-130
  ...
  retransmitted to `FF0X::B:FFFB`. The retry endpoint TTL for BRC-132 frames is
  longer than the transaction-frame TTL, to accommodate large reassembly
  windows; the reference implementation defaults it to five minutes.

### Merkle Verification

After reassembly, a consumer MAY recompute the Merkle root to verify the
SubtreeID:

- The consumer decodes the payload into nodes and computes SHA256d pairwise up
  the binary tree, then compares the root to SubtreeID.
- On mismatch, the reassembled object MUST be dropped.
- The cost is significant at scale (roughly one double-SHA256 per node, so
  ~1M operations for a 1M-node subtree), which is why this is optional rather
  than required, and why an implementation that offers it SHOULD leave it off
  by default.
