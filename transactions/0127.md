...
1. For each (SubtreeID, GroupID) pair, construct a 64-byte SubtreeGroupAnnounce
   datagram.
2. Deliver the datagram to the SubtreeGroupAnnounce control group on the
   configured multicast scope(s). A sender on the multicast fabric sends it as
   a UDP packet directly. A sender that is not on the fabric, or that has no
   multicast egress of its own, MAY instead submit the same 64 bytes to an
   ingress proxy over its submission transport and have the proxy forward them
   to the control group unchanged. The datagram on the control group is
   identical either way, so a listener cannot tell the two apart and needs no
   knowledge of which was used.
