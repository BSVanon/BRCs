...
This BRC specifies how standalone 80-byte BSV block headers are distributed to
downstream consumers, in two forms. The **datagram form** wraps the header in a
minimal 172-byte frame that reuses the 92-byte BRC-124 header layout, ensuring
compatibility with existing infrastructure classifiers and firewall rules, and
is carried over IPv6 multicast or unicast datagrams. The **stream form** writes
the bare 80-byte header, with no frame at all, on a single-class byte stream
dedicated to headers. Both are produced by splitting the block header out of a
BRC-131 block announcement.
...
This section governs the **datagram form**; the stream form has no group and is
specified under *Stream Delivery Form* below.
...
1. Extract the 80-byte block header from `BlockAnnounce` payload bytes `[0:80]`
   — the first 80 bytes of the BRC-144 block object that payload carries.
...
### Stream Delivery Form

On a **single-class byte stream** carrying nothing but block headers, the
emitter writes the bare 80-byte header and nothing else: no frame header, no
tag, no length prefix, no record envelope. A block header is fixed size, so the
stream is self-delimiting — a consumer reads 80 bytes at a time, and the Nth
header begins at byte offset `80 × (N − 1)`.

This is the form for a persistent unicast connection dedicated to headers, where
the connection itself identifies the class. It is deliberately minimal:

- **No frame header.** The 92-byte header exists to classify a datagram on a
  shared multicast group and to carry the emitter's sequencing. A dedicated
  stream needs neither: the connection names the class, and the transport
  already orders and delivers what is written.
- **No sequencing fields.** `HashKey` and `SeqNum` identify the emitter's flow,
  which is meaningless to a subscriber and differs between the primary and
  standby emitters that may serve it. What a consumer verifies is the header
  itself, and the chain it builds from them.
- **No length prefix.** Eighty bytes is fixed by consensus. A length would be
  redundant, and a reader that trusted it over the fixed size would desynchronise
  on a malformed writer rather than on the first bad header.

A consumer of the stream form:

1. **Read** exactly 80 bytes.
2. **Chain** — check that `PrevBlockHash` (bytes 4–35) equals `SHA256d` of the
   previous header it accepted, which is what makes a stream of headers a chain
   rather than a list.
3. **Identify** — compute `BlockHash = SHA256d(header)` where the block id is
   needed; the stream carries no separate identifier.

A stream is a **live tail**: a consumer that connects mid-chain receives headers
from connect time forward and cannot bootstrap a chain from this stream alone.

The two forms carry the same 80 bytes and differ only in envelope, so an emitter
MAY serve both, and a consumer MUST NOT expect frame fields on the stream form.

### Consumer Processing

A consumer receiving BRC-135 **datagram** frames:
