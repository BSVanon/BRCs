...
3. Listeners reassemble fragments keyed on the Transaction ID, and MAY verify
   the reassembled payload against that identifier before forwarding.
...
The 32-byte canonical transaction identifier of the complete unfragmented
payload. For a standard (BRC-12 or BRC-124) payload this is the double-SHA256
...
For an Extended Format payload ([BRC-128](./0128.md)) it is **not** the hash of
the payload bytes. EF carries the marker and per-input source data that the
identifier does not cover, so the identifier is computed over the standard
serialization of the transaction, exactly as it is everywhere else in Bitcoin.
Hashing the EF bytes directly yields a value that will never match.

The same TxID appears on every fragment of a transaction. It serves as the
reassembly key, and as an integrity proof a listener MAY check after
reassembly. Verification is OPTIONAL rather than required: at line rate it
costs a full pass over every reassembled payload, and an implementation that
offers it SHOULD make it configurable. A listener that verifies MUST drop the
object on mismatch.
